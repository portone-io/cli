...
function copyBinaryToNativePackage(platform: string, arch: string) {
  ...
  // Copy the portone-<target>[.exe] binary that CI placed at the repository root.
  const ext = os === "win32" ? ".exe" : "";
  const binarySource = resolve(
    REPO_ROOT,
    `${getName(platform, arch, "portone")}${ext}`,
  );
  const binaryTarget = resolve(packageRoot, `portone${ext}`);
